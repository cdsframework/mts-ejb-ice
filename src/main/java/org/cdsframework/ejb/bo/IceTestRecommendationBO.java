 ...
 */
@Stateless
public class IceTestRecommendationBO extends BaseBO<IceTestRecommendationDTO> {

    @EJB
    private IceTestProposalBO iceTestProposalBO;
     ...
     */
    private void updateParentsLastModDatetime(IceTestRecommendationDTO baseDTO, SessionDTO sessionDTO, PropertyBagDTO propertyBagDTO)
        ...
        if (!overrideCreateModProperties) {
            try {
                if (baseDTO != null) {
                    IceTestProposalDTO parentDTO = new IceTestProposalDTO();
                    parentDTO.setProposalId(baseDTO.getProposalId());
                    parentDTO = iceTestProposalBO.findByPrimaryKeyMain(parentDTO, new ArrayList<Class>(), sessionDTO, propertyBagDTO);
                    if (parentDTO != null) {
                        parentDTO.setLastModDatetime(baseDTO.getLastModDatetime());
                        logger.debug(METHODNAME, "parent BO update called!");
                        iceTestProposalBO.updateMain(parentDTO, Update.class, sessionDTO, propertyBagDTO);
                    } else {
     ...
     */
    @Override
    protected void processEnd(IceTestRecommendationDTO baseDTO, List<Class> childClassDTOs, Operation operation, Class queryClass, SessionDTO sessionDTO, PropertyBagDTO propertyBagDTO)
